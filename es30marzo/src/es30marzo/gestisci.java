...
public class gestisci {
    private ArrayList<giocatore> list = new ArrayList<giocatore>();
    ...
    public void crea(){
        try {
            ...
            for(int i = 0; i<n; i++)F.writeObject(new giocatore());
            f.flush();
            f.close();
        } 
        catch (Exception e) {
        ...
        try {
            ...
            while(true){
                try {
                    ...
                    if(temp.getSquadra().equals(squadra))temp.stampa_nome();
                } 
                catch (EOFException e) {
                    break;
                }
    ...
    public void copia_con_arr(){
        try {
            ...
            // Inserimento giocatori nel file
            for(giocatore i: list)F.writeObject(i);
            f.flush();
            f.close();
            list.clear();
        } 
    ...
    public void cancella_giocatore(){
        copia_su_arr();
        ...
        try {
            ...
            for(giocatore i: list){
                if(!(i.getCognome().equals(cognome) && i.getNome().equals(nome)))F.writeObject(i);
                else JOptionPane.showMessageDialog(null, "Elemento rimosso con successo!");
            } 
            f.flush();
            f.close();
            list.clear();
